...
export default async function CollectionPage({ params, searchParams }: CollectionPageProps) {
  ...
  return (
    <main className="min-h-screen w-full bg-black text-white px-4 sm:px-8 lg:px-16 py-32 relative selection:bg-gold/30">
      ...
      <div className="w-full max-w-[1900px] min-w-[278px] mx-auto relative z-10 flex flex-col gap-12">
        ...
        <header className="border-b border-white/5 pb-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          ...
          <div className="flex flex-wrap gap-2 font-mono text-[9px] tracking-widest font-bold w-full md:w-auto">
            <Link 
              ...
              className={`px-4 py-2 border transition-all duration-300 cursor-pointer text-center flex-1 sm:flex-none ${!mine ? 'border-gold text-gold bg-gold/5' : 'border-white/5 text-zinc-400 hover:border-gold hover:text-gold'}`}
            >
            ...
            {targetMines.map((m) => (
              <Link
                ...
                className={`px-4 py-2 border uppercase transition-all duration-300 cursor-pointer text-center flex-1 sm:flex-none ${mine === m ? 'border-gold text-gold bg-gold/5' : 'border-white/5 text-zinc-400 hover:border-gold hover:text-gold'}`}
              >
                {m}
              </Link>
            ))}
        ...
        {/* GRILLA CON RESPONSIVE PROGRESIVO BALANCEADO (Sin desbordamiento lateral) */}
