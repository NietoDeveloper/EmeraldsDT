...
/**
 * Emerald DT - Home Page (Ciclo S+)
 * Arquitectura de alto rendimiento con Snap Scroll y diseño cinemático.
 */
export default async function HomePage({ params }: HomePageProps) {
  ...
  return (
    <main className="relative w-full bg-black snap-y snap-mandatory h-screen overflow-y-auto scrollbar-hide selection:bg-gold/30">
      
      {/* SECCIÓN 1: HERO (SpaceX Style) */}
      ...
      {mines.map((mine, index) => (
        ...
        >
          {/* Fondo: Optimización de Asset y Contraste */}
          <div className="absolute inset-0 z-0">
            {/* Gradiente de profundidad para legibilidad del texto */}
      ...
      {/* SECCIÓN FINAL: TECNOLOGÍA (Nieto Lab Engineering) */}
        ...
        <div className="container mx-auto px-6 sm:px-12 md:px-24 lg:px-32 max-w-[1900px] z-10">
          <div className="grid md:grid-cols-2 gap-20 items-center">
            <div className="relative">
              ...
              <div className="space-y-6 border-l border-emerald-500/50 pl-8">
                ...
                <p className="text-zinc-500 max-w-md text-sm md:text-base leading-relaxed">
                  {isEs
                    ? 'Arquitectura de seguridad máxima y trazabilidad absoluta para cada activo de alto valor en la cadena de suministros de esmeraldas colombianas.'
                    : 'Maximum security architecture and absolute traceability for every high-value asset in the Colombian emerald supply chain.'}
                </p>
              </div>
            </div>
            
            {/* Visual Technical Box */}
            <div className="hidden md:block h-[350px] border border-white/10 bg-gradient-to-br from-white/5 to-transparent backdrop-blur-sm relative group overflow-hidden">
              ...
              <div className="absolute top-4 left-4 w-1 h-1 bg-emerald-500 animate-pulse" />
            </div>
          </div>
        </div>
      </section>
