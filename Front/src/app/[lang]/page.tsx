...
export default async function HomePage({ params }: HomePageProps) {
  ...
  const mines = [
    ...
    },
    {
      ...
      descEs: 'Conocidas por producir gemas con un brillo excepcional y un tiinto verde-amarillento que captura la luz del sol de los Andes colombianos.',
          ...
          <div className="w-full px-6 sm:px-12 md:px-24 lg:px-32 xl:px-48 relative z-20 max-w-[1900px] mx-auto">
            <div className="animate-fade-in-up duration-1000">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                {/* 🛰️ ENLACE DE RED DIRECCIONADO AL CATÁLOGO DE JOYAS ÚNICAS */}
                <Link 
                  href={`/${lang}/collection?mine=${mine.id}`}
                  className="w-full sm:w-auto"
                >
                  <MainButton 
                    text={isEs ? 'VER COLECCIÓN' : 'VIEW COLLECTION'} 
                    variant="gold" 
                    className="w-full border-2 border-gold text-gold hover:bg-gold hover:text-black font-bold py-4 px-12 transition-all duration-500 tracking-widest text-[11px]"
                  />
                </Link>
              </div>
            </div>
