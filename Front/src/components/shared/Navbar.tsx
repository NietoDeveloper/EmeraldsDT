    ...
    <>
      <nav 
        className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] 
          ...
          ${isAtTop 
            ? "bg-transparent mt-1 md:mt-6 py-6 md:py-10" 
      ...
      >
        <div className="w-full max-w-[1900px] mx-auto flex justify-between items-center px-6 sm:px-12 md:px-24 lg:px-32 xl:px-48 transition-all duration-500">
          ...
          <Link href="/" className="group flex items-center gap-3 md:gap-5 z-[120] outline-none cursor-pointer flex-shrink-0">
            ...
            {/* Ajuste de tamaño Emerald y DT: Ambos en text-lg md:text-3xl */}
          ...
          <div className="hidden lg:flex items-center gap-10 xl:gap-16">
            {navLinks.map((link) => (
              <div key={link.name} className="relative group/item">
                <Link 
                  href={link.href} 
                  className="flex items-center gap-2 text-[11px] xl:text-[13px] uppercase tracking-[0.4em] xl:tracking-[0.5em] font-black text-emerald/80 hover:text-gold hover:-translate-y-0.5 transition-all duration-500 py-2"
                >
                  {link.name}
          ...
          <div className="flex items-center gap-4 md:gap-10">
            <div className="hidden md:flex items-center gap-8 border-l border-emerald/30 pl-10 text-emerald">
              <Link href="/cart" className="hover:text-gold transition-all hover:scale-125 hover:-translate-y-1 inline-block">
                <ShoppingCart size={24} />
              </Link>
              <Link href="/login" className="text-[12px] uppercase tracking-[0.3em] font-black hover:text-gold hover:-translate-y-0.5 transition-all">
                Log In
              </Link>
            </div>

            {/* Hamburguesa con color Emerald y hover Gold */}
            <button 
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              className="lg:hidden relative z-[150] cursor-pointer group outline-none overflow-visible"
              aria-label="Toggle Menu"
            >
              <div className="relative w-12 h-12 md:w-16 md:h-16 flex items-center justify-center bg-emerald/5 rounded-full border border-emerald/20 group-hover:border-gold group-hover:-translate-y-1 transition-all duration-500">
                <Menu 
                  size={28} 
